...
describe('Database',function(){
    ...
    var store = jasmine.createSpyObj('store',['create', 'add','read']);

    beforeEach(module('database', function($provide){
        inflateSpy  = jasmine.createSpy('inflate').and.callFake(function(){return arguments[0];});
        templateSpy = jasmine.createSpy('template').and.callFake(function(){return arguments[0];});
    ...
    beforeEach(inject(['Database',function(Database){
        db = new Database();
    }]));
    ...
    describe(': inflate', function(){
        ...
        it('should not template if query has been interpolated', function(){
            ...
            db.inflate('ns', query, context);
            expect(templateSpy.calls.count()).toBe(1*2); // once for the query, once for nsData
        });
        ...
        it('should force template if regenerateTemplate is true', function(){
            ...
            db.inflate('ns', query, context);
            db.inflate('ns', query, context);
            expect(templateSpy.calls.count()).toBe(2*2); // once for the query, once for nsData
        });
...
describe('Sequence', function(){
    ...
    beforeEach(inject(function(databaseSequence){
        db = {
            inflate:jasmine.createSpy('inflate').and.callFake(function(namespace, obj){return obj;})
        };
